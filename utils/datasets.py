...
def normalize_rewards(dataset, target_min=-1.0, target_max=0.0):
    r = np.asarray(dataset['rewards'])
    r_min, r_max = r.min(), r.max()
    
    # [r_min, r_max] → [target_min, target_max]
    new_rewards = (r - r_min) / (r_max - r_min) * (target_max - target_min) + target_min
    new_rewards = new_rewards.astype(np.float32)
    new_rewards.setflags(write=False)
    
    # FrozenDict은 .copy(add_or_replace=...)로 새 인스턴스를 만듭니다
    new_data = dict(dataset)
