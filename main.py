...
flags.DEFINE_string('env_name', 'humanoidmaze-large-navigate-singletask-v0', 'Environment (dataset) name.')
...
config_flags.DEFINE_config_file('agent', 'agents/lac.py', lock_config=False)
...
def main(_):
    ...
    run = setup_wandb(project='LAC', group=task_id, name=f"{base_env}_L{config['critic_num_layers']}")
    ...
    for i in tqdm.tqdm(range(1, FLAGS.offline_steps  +1)):
        ...
        if i == FLAGS.offline_steps - 1 or \
            ...
            for idx, video in enumerate(renders):
                imageio.mimsave(
                    os.path.join(video_dir, f"eval_{log_step}_{idx}.mp4"),
                    video,
                    fps=30,
                )
...
def run_all_tasks_repeat(_):
    base_env_name = FLAGS.env_name
    repeat = 3

    # singletask-v0 → task1~5
    if base_env_name.endswith("-singletask-v0"):
        env_names = [
            base_env_name.replace(
                "-singletask-v0",
                f"-singletask-task{i}-v0"
            )
            for i in range(1, 6)
        ]
    else:
        env_names = [base_env_name]

    for r in range(repeat):
        FLAGS.seed = random.randint(0, 2**31 - 1)
        print(f"\n########## REPEAT {r + 1}/{repeat} ##########\n", flush=True)

        for env_name in env_names:
            FLAGS.env_name = env_name
            print(f"===== Running {env_name} | repeat={r + 1} =====", flush=True)

            main(None)             
            wandb.finish()          
            jax.clear_caches()      

def run_one_tasks_repeat(_):
    repeat = 1
    ...
    for r in range(repeat):
        ...
        print(f"===== Running main | repeat={r + 1} =====", flush=True)
...
if __name__ == '__main__':
    # app.run(main)
    # app.run(run_all_tasks_repeat)
    app.run(run_one_tasks_repeat)